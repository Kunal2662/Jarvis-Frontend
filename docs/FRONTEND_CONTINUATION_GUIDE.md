...
**LAST COMPLETED FRONTEND STEP:** Step 17 — Agents (`/agents`, a new `AgentService` seam — `agentService.ts` — observability + local enable/disable only, deliberately no run/execute method). A genuine scope correction was made before writing any code: `docs/JARVIS_CORE_FRONTEND_MAPPING.md`'s own row for Agents reads "Expose existing orchestration; **no second agent framework**" — every "agent" mention elsewhere in this checkpoint ties to the single, existing `AgentOrchestrator` behind Chat/Voice (Steps 4-5), the old v1 `/agents` route already redirected to `/chat`, and `Automation.actions` (Step 8) already models "running an agent" as a Core-owned consequence, not a frontend-triggered action. `Agent { id, name, description, status: 'active'|'idle'|'disabled', capabilities, lastRunAt? }` with a separate read-only `AgentRun` activity history (`'completed'|'failed'` only — never a live run this UI itself initiated). 5 seeded fictional agent roles (Research/Daily Briefing/System Health/Task Planning/Smart Home Agent). No run/execute action, no agent creation/configuration UI, no real permission enforcement. Registered as a fifth Universal Search category (`'agent'`). The old v1 `/agents → /chat` redirect was removed now that `/agents` has a real page. No verified Core Agents contract was found in this repository — see `docs/CORE_AGENTS_CONTRACT_REQUIRED.md`.

**NEXT FRONTEND STEP:** Settings (`docs/FRONTEND_IMPLEMENTATION_ROADMAP.md` Phase 8, item 19 — the next unstarted item now that Phase 7 (items 16-17) is complete. Roadmap item 18 "Automations" is a duplicate reference to the already-complete Step 8, not a new item. No Core contract is documented anywhere in this checkpoint — verify before implementing, per the roadmap's standing rule.).

Do not start Settings automatically when merely reading this document — wait for explicit approval/instruction, and re-read `docs/FRONTEND_PROGRESS.md` first.
...
13. Agents — frontend done (in-memory mock adapter, observability + local enable/disable via a new `AgentService` seam, no run/execute action per "no second agent framework"), Core AgentOrchestrator integration pending ← just completed (Step 17); Phase 7 is now fully built as a frontend surface, next up is Settings (Phase 8, item 19)
